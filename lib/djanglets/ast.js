...
computeBlockTypes = ["if", "elif"];
signalBlockTypes = ["else", "endif"];
...
visitor = {
  ...
  visitComputeBlock: function (ast, type, variable) {
    console.log("visitComputeBlock", type, variable);
    if (computeBlockTypes.indexOf(type) === -1) {
      throw new Error(
        "Invalid compute block type. Supported types: " + computeBlockTypes.join(","));
    }
  },
  visitSignalBlock: function (ast, type) {
    console.log("visitSignalBlock", type);
    if (signalBlockTypes.indexOf(type) === -1) {
      throw new Error(
        "Invalid compute block type. Supported types: " + signalBlockTypes.join(","));
    }
  },
};
