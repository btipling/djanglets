...
visitor = {
  ...
  visitSelfClosingElement: function (ast, type) {
    this.visitOpenElement(ast, type);
  ...
  visitAttributeContents: function (ast, contents, content) {
    var prev = contents[contents.length - 1];
    if (typeof contents[0] === "string" && typeof prev === "string") {
      prev += content[0];
      contents[contents.length -1] = prev;
      return contents;
    }
    return contents.concat(content);
  },
