...
rules: [/^(?:\s+)/,/^(?:\s+)/,/^(?:<(?=\w+))/,/^(?:<\/(?=\w+))/,/^(?:>)/,/^(?:\/>)/,/^(?:["\"'"])/,/^(?:["\"'"])/,/^(?:[^"\"'"]+)/,/^(?:=)/,/^(?:<!--)/,/^(?:-->)/,/^(?:\s)/,/^(?:.)/,/^(?:\{\{\s*)/,/^(?:\s*\}\})/,/^(?:\{%\s*)/,/^(?:%\})/,/^(?:\s*,)/,/^(?:\w+)/,/^(?:\\\{)/,/^(?:.)/,/^(?:$)/,/^(?:([^"<"\s])+)/],
conditions: {"comment":{"rules":[11,12,13,22],"inclusive":true},"escape":{"rules":[21,22],"inclusive":true},"tag":{"rules":[0,4,5,6,9,14,16,19],"inclusive":false},"var":{"rules":[15,19],"inclusive":false},"attribval":{"rules":[7,8],"inclusive":false},"djtag":{"rules":[1,17,18,19],"inclusive":false},"INITIAL":{"rules":[1,2,3,10,14,16,20,22,23],"inclusive":true}}
});
return lexer;
})();
parser.lexer = lexer;
